...
uint32_t prevTempLimitMillis = 0;
uint32_t prevSendTempMillis = 0;
int httpCounter = 0;
...
const int FETCH_TEMP_LIMITS_INTERVAL = 15000;
const int SEND_TEMP_INTERVAL = 10000;
//------------------------------------------------------------------------
int sensorId = 2;
...
void loop()
{
  ...
  if (currentMillis - prevTempLimitMillis >= FETCH_TEMP_LIMITS_INTERVAL) {
    prevTempLimitMillis = currentMillis;
    getTempLimit();
  }

  if (currentMillis - prevSendTempMillis >= SEND_TEMP_INTERVAL) {
    prevSendTempMillis = currentMillis;
    collectTemperature();
  }
